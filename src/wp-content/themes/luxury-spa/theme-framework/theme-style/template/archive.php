...
<article id="post-<?php the_ID(); ?>" <?php post_class('cmsmasters_archive_type'); ?>>
	...
	if (!post_password_required() && has_post_thumbnail()) {
		echo '<div class="cmsmasters_archive_item_img_wrap">';
			...
			luxury_spa_thumb(get_the_ID(), 'cmsmasters-square-thumb');
	...
	<div class="cmsmasters_archive_item_cont_wrap">
		...
		if (get_post_type() == 'post') {
			echo '<span class="cmsmasters_archive_item_user_name"> ' . 
					'<a href="' . esc_url(get_author_posts_url(get_the_author_meta('ID'))) . '" rel="author" title="' . esc_attr__('Posts by', 'luxury-spa') . ' ' . get_the_author_meta('display_name') . '">' . get_the_author_meta('display_name') . '</a>' . 
				'</span>' . 
		...
		if (luxury_spa_excerpt(55, false) != '') {
			echo cmsmasters_divpdel('<div class="cmsmasters_archive_item_content entry-content">' . "\n" . 
				wpautop(luxury_spa_excerpt(55, false)) . 
			'</div>' . "\n");
		}
		
		echo '<a class="cmsmasters_archive_read_more" href="' . esc_url(get_permalink(get_the_ID())) . '">' . esc_html__('Read More', 'luxury-spa') . '</a>';
		?>
	</div>
</article>