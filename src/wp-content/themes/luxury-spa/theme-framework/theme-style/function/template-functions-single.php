...
function luxury_spa_prev_next_posts($cmsmasters_post_type_name = '', $order_cat = false, $tax_name = 'category') {
	...
	if ($published_posts > 1) {
		...
		previous_post_link('<span class="cmsmasters_prev_post"><span class="post_nav_sub">' . esc_html__('Previous', 'luxury-spa') . '<span class="post_nav_type"> ' . esc_html($cmsmasters_post_type_name) . ' </span></span>%link<span class="cmsmasters_prev_arrow"><span></span></span></span>', '%title',  $order_cat, '', $tax_name);
		
		next_post_link('<span class="cmsmasters_next_post"><span class="post_nav_sub">' . esc_html__('Next', 'luxury-spa') . '<span class="post_nav_type"> ' . esc_html($cmsmasters_post_type_name) . ' </span></span>%link<span class="cmsmasters_next_arrow"><span></span></span></span>', '%title',  $order_cat, '', $tax_name);
...
function luxury_spa_sharing_box($title_box = false, $tag = 'h3') {
...
function luxury_spa_author_box($title_box = false, $tag = 'h3', $author_tag = 'h4') {
	...
	if ($user_description) {
		...
		if ($user_url) {
			echo '<a class="about_author_site" href="' . esc_url($user_url) . '" title="' . esc_attr(get_the_author()) . ' ' . esc_attr__('website', 'luxury-spa') . '" target="_blank">' . esc_html($user_url) . '</a>';
		}
...
function luxury_spa_related($tag = 'h3', $title = '', $no_title = '', $box_type = false, $tgsarray = null, $items_number = 5, $pause_time = 5, $type = 'post', $taxonomy = null) {
	if ( 
		...
		if ($r_query->have_posts()) {
			echo "<aside class=\"cmsmasters_single_slider\">";
				echo "<" . esc_html($tag) . " class=\"cmsmasters_single_slider_title\">" . 
					($title != '' ? esc_html($title) : esc_html__('More items', 'luxury-spa')) . 
				"</" . esc_html($tag) . ">";
						...
						while ($r_query->have_posts()) : $r_query->the_post();
							echo "<div class=\"cmsmasters_owl_slider_item cmsmasters_single_slider_item\">
								<div class=\"cmsmasters_single_slider_item_outer\">";
									...
									luxury_spa_thumb(get_the_ID(), 'cmsmasters-project-masonry-thumb', true, false, true, false, true, true, false, false, false, 'cmsmasters_theme_icon_image');
...
function luxury_spa_author_avatar($template_type = 'page') {
...
function luxury_spa_get_pings($id, $tag = 'h3') {
	$out = '';
	...
	if (sizeof($pings) > 0) {
		$out .= '<aside class="cmsmasters_pings_list">' . "\n" .
			'<' . esc_html($tag) . '>' . esc_html__('Trackbacks and Pingbacks', 'luxury-spa') . '</' . esc_html($tag) . '>' . "\n" .
