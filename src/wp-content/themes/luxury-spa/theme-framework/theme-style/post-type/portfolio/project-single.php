...
if (
	$cmsmasters_option['luxury-spa' . '_portfolio_project_like'] ||
	$cmsmasters_option['luxury-spa' . '_portfolio_project_date'] ||
	$cmsmasters_option['luxury-spa' . '_portfolio_project_cat'] ||
	$cmsmasters_option['luxury-spa' . '_portfolio_project_comment'] ||
	$cmsmasters_option['luxury-spa' . '_portfolio_project_author'] ||
	$cmsmasters_option['luxury-spa' . '_portfolio_project_tag'] ||
...
<article id="post-<?php the_ID(); ?>" <?php post_class('cmsmasters_open_project'); ?>>
	...
	if ($cmsmasters_post_format == 'gallery') {
		...
		luxury_spa_post_type_gallery(get_the_ID(), $cmsmasters_project_images, $cmsmasters_project_columns, 'cmsmasters-project-masonry-thumb', 'cmsmasters-project-full-thumb');
		...
		luxury_spa_post_type_video(get_the_ID(), $cmsmasters_project_video_type, $cmsmasters_project_video_link, $cmsmasters_project_video_links, 'cmsmasters-project-full-thumb');
		...
		if ($cmsmasters_project_image_show == 'true' && $cmsmasters_project_images[0] == '') {
			...
			luxury_spa_post_type_slider(get_the_ID(), $cmsmasters_project_images, 'cmsmasters-full-masonry-thumb');
		}
	}
	...
	if ($cmsmasters_project_title == 'true') {
		echo '<header class="cmsmasters_project_header entry-header">';
			luxury_spa_project_title_nolink(get_the_ID(), 'h1');
		echo '</header>';
	}
	...
	echo '<div class="project_content' . (($project_sidebar == 'true') ? ' with_sidebar' : '') . '">';
		...
		if (get_the_content() != '') {
			echo '<div class="cmsmasters_project_content entry-content">' . "\n";
				...
				wp_link_pages(array(
					'before' => '<div class="subpage_nav">' . '<strong>' . esc_html__('Pages', 'luxury-spa') . ':</strong>',
					'after' => '</div>',
					'link_before' => '<span>',
					'link_after' => '</span>'
				));
			...
			echo '<div class="project_sidebar">';
				...
				if ($project_details == 'true') {
					echo '<div class="project_details entry-meta">' .
						...
						luxury_spa_get_project_likes('post');

						luxury_spa_get_project_comments('post');

						luxury_spa_get_project_author('post');

						luxury_spa_get_project_date('post');

						luxury_spa_get_project_category(get_the_ID(), 'pj-categs', 'post');

						luxury_spa_get_project_tags(get_the_ID(), 'pj-tags');

						luxury_spa_get_project_features('details', $cmsmasters_project_features, false, 'h4', true);

						luxury_spa_project_link($cmsmasters_project_link_text, $cmsmasters_project_link_url, $cmsmasters_project_link_target);

					echo '</div>';
				}


				luxury_spa_get_project_features('features', $cmsmasters_project_features_one, $cmsmasters_project_features_one_title, 'h4', true);

				luxury_spa_get_project_features('features', $cmsmasters_project_features_two, $cmsmasters_project_features_two_title, 'h4', true);

				luxury_spa_get_project_features('features', $cmsmasters_project_features_three, $cmsmasters_project_features_three_title, 'h4', true);
...
if ($cmsmasters_project_sharing_box == 'true') {
	luxury_spa_sharing_box();
}


if ($cmsmasters_option['luxury-spa' . '_portfolio_project_nav_box']) {
	$order_cat = (isset($cmsmasters_option['luxury-spa' . '_portfolio_project_nav_order_cat']) ? $cmsmasters_option['luxury-spa' . '_portfolio_project_nav_order_cat'] : false);

	luxury_spa_prev_next_posts(__('Project', 'luxury-spa'), $order_cat, 'pj-categs');
}


if ($cmsmasters_project_author_box == 'true') {
	luxury_spa_author_box(esc_html__('About author', 'luxury-spa'), 'h2', 'h3');
}
...
if ($cmsmasters_project_more_posts != 'hide') {
	luxury_spa_related(
		'h2',
		esc_html__('More projects', 'luxury-spa'),
		esc_html__('No projects found', 'luxury-spa'),
		$cmsmasters_project_more_posts,
		$tgsarray,
		$cmsmasters_option['luxury-spa' . '_portfolio_more_projects_count'],
		$cmsmasters_option['luxury-spa' . '_portfolio_more_projects_pause'],
		'project',
		'pj-tags'
	);
}
