...
$excerpt = (in_array('excerpt', $cmsmasters_project_metadata) && luxury_spa_project_check_exc_cont()) ? true : false;
...
<article id="post-<?php the_ID(); ?>" <?php post_class('cmsmasters_project_grid'); echo (($project_categs != '') ? ' data-category="' . esc_attr($project_categs) . '"' : '') ?>>
	<div class="project_outer">
		...
		<?php
			luxury_spa_thumb_rollover(get_the_ID(), $project_thumb_size, $rollover, true, false, false, false, false, false, $project_thumb_high, true, $cmsmasters_project_link_redirect, $cmsmasters_project_link_url, $cmsmasters_project_link_target);
		...
		if ($title || $excerpt || $likes || $comments) {
			echo '<div class="project_inner">';
				...
				$title ? luxury_spa_project_heading(get_the_ID(), 'h3', $cmsmasters_project_link_redirect, $cmsmasters_project_link_url, $cmsmasters_project_link_target) : '';

				$excerpt ? luxury_spa_project_exc_cont() : '';


				if ($likes || $comments) {
					echo '<footer class="cmsmasters_project_footer entry-meta">';
						luxury_spa_project_more(get_the_ID(), $cmsmasters_project_link_redirect, $cmsmasters_project_link_url, $cmsmasters_project_link_target);

						$comments ? luxury_spa_get_project_comments('page') : '';

						$likes ? luxury_spa_get_project_likes('page') : '';
		...
		if (!$title) {
			echo '<div class="dn">';
				luxury_spa_project_heading(get_the_ID(), 'h6');
			echo '</div>';
		}
