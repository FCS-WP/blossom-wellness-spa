...
function luxury_spa_get_admin_page() {
	global $pagenow;
...
function luxury_spa_default_tab() {
	$current_page = luxury_spa_get_admin_page();
...
function luxury_spa_get_the_tab() {
	global $pagenow;
	
	$default_tab = luxury_spa_default_tab();
...
function luxury_spa_settings_page_header() {
	...
	if ($tabs != '') {
		...
		if (class_exists('Cmsmasters_Custom_Fonts') && $page == 'cmsmasters-settings-font') {
			$links[] = '<a class="' . $class . ' cmsmasters_recaptcha_nav_tab' . '" href="edit.php?post_type=cmsmasters_font">' . esc_html__('Custom Fonts', 'luxury-spa') . '</a>';
		}
