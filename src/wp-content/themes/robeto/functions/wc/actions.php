...
if ( NOVA_WOOCOMMERCE_IS_ACTIVE ) {
	...
	function woocommerce_template_loop_stock() {
	    ...
	    if ( ! $product->is_in_stock() )
	        echo '<span class="stock out-of-stock">' . esc_html__( 'Out of stock', 'robeto' ) . '</span>';
	}
				...
				<form class="header_search_form" role="search" method="get" action="' . esc_url( home_url( '/'  ) ) .'">
					...
					<div class="header_search_input_wrapper">
						<input
							...
							data-min-chars="3"
							placeholder="' . esc_attr__( 'Product Search', 'robeto' ) . '"
							/>
						...
						if( $categories ) {
							...
							echo '<div class="header_search_select_wrapper">
									<select name="search_category" id="header_search_category_'.$rand_id.'" class="header_search_select">
										<option value="all" selected>' . esc_html__( 'Select Category', 'robeto' ) . '</option>';
	...
	function nova_category_large_icons() {
		if ( !NOVA_WOOCOMMERCE_IS_ACTIVE ) return;
			...
			if ( nova_new_products_page_url() !== false ):
				echo '<a href="' . nova_new_products_page_url() . '"><i class="robeto-icons-ui_star"></i><span>'. nova_new_products_title('') .'</span></a>';
			endif;

			if ( nova_sale_page_url() !== false ):
				echo '<a href="'.nova_sale_page_url().'"><i class="robeto-icons-ecommerce_discount-symbol"></i><span>'. nova_on_sale_products_title('') .'</span></a>';
			endif;
...
if( ! function_exists( 'nova_shop_filters' ) ){
	function nova_shop_filters() {
		...
		<div id="side-filters" class="nova-product-filter-content">
				<header>
					<h2><?php echo esc_html__( 'Filters','robeto' ); ?></h2>
					<button class="js-product-filters-toogle"><?php echo esc_html__( 'Close','robeto' ); ?></button>
				</header>
...
if( ! function_exists( 'nova_toggle_registration_login' ) ){
	function nova_toggle_registration_login($context) {
		...
		if ( $context == 'login' ) { ?>
			...
			<p class="form-actions extra"><?php esc_html_e('Already a member?', 'robeto'); ?><a href="#nova-login-wrap" class="login-link"><?php esc_html_e('Login', 'robeto'); ?></a></p>

		<?php } else if ( $context == 'register' ) { ?>

			<p class="form-actions extra"><?php esc_html_e('Not a member?', 'robeto'); ?><a href="#nova-register-wrap" class="register-link"><?php esc_html_e('Register', 'robeto'); ?></a></p>
