...
function nova_breadcrumbs( $args = '' ) {
	$args = wp_parse_args( $args, array(
		...
		'show_on_front'     => false,
		'labels'            => array(
			'home'      => esc_html__( 'Home', 'robeto' ),
			'archive'   => esc_html__( 'Archives', 'robeto' ),
			'blog'      => esc_html__( 'Blog', 'robeto' ),
			'search'    => esc_html__( 'Search results for', 'robeto' ),
			'not_found' => esc_html__( 'Not Found', 'robeto' ),
			'author'    => esc_html__( 'Author Archives:', 'robeto' ),
			'day'       => esc_html__( 'Daily Archives:', 'robeto' ),
			'month'     => esc_html__( 'Monthly Archives:', 'robeto' ),
			'year'      => esc_html__( 'Yearly Archives:', 'robeto' ),
		),
	) );
	...
	elseif ( is_day() ) {
		$items[] = sprintf(
			$item_text_tpl,
			sprintf( esc_html__( '%s %s', 'robeto' ), $args['labels']['day'], get_the_date() )
		);
	} // Month archive
	elseif ( is_month() ) {
		$items[] = sprintf(
			$item_text_tpl,
			sprintf( esc_html__( '%s %s', 'robeto' ), $args['labels']['month'], get_the_date( 'F Y' ) )
		);
	} // Year archive
	elseif ( is_year() ) {
		$items[] = sprintf(
			$item_text_tpl,
			sprintf( esc_html__( '%s %s', 'robeto' ), $args['labels']['year'], get_the_date( 'Y' ) )
		);
	} // Archive
	else {
		$items[] = sprintf(
